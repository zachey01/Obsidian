...
class WindowsWindowEffect:
    ...
    def removeAeroEffect(self, hWnd):
        """Удаляет акриловый эффект с окна"""
        self.winCompAttrData.Attribute = 19
        self.accentPolicy.AccentState = 0  # ACCENT_DISABLED
        self.SetWindowCompositionAttribute(hWnd, pointer(self.winCompAttrData))

    def setAeroEffect(self, hWnd):
        """Применяет акриловый эффект к окну"""
        self.winCompAttrData.Attribute = 19
        self.accentPolicy.AccentState = 3  # ACCENT_ENABLE_BLURBEHIND
        self.accentPolicy.AccentFlags = 2  # Включить размытие
        self.accentPolicy.GradientColor = 0x01000000  # Прозрачность
        self.SetWindowCompositionAttribute(hWnd, pointer(self.winCompAttrData))
